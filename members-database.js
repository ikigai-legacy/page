
const profiles = [{
    dataid: '1',
    image: 'profiles/nabi.png',
    name: '𝒊𝒌𝒈_𝒏𝒂𝒃𝒊',
    dataname: 'ikg_nabi',
    role: '𝒇𝒐𝒖𝒏𝒅𝒆𝒓',
    birthday: '𝒅𝒆𝒄𝒆𝒎𝒃𝒆𝒓 𝟐',
    databirthday: '2022-12-02',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',  
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_nabi" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@ikg_nabi" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@nabi_ikigai" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:3q933xqj" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@kyiiopta" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@ikg_nabi" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@ikg_nabi" }
    ],
    music: {
        title: "✩°｡⋆⸜ 🦢✮",
        artist: "@𝒌𝒚𝒊𝒊𝒐𝒑𝒕𝒂",
        cover: "spotify/icons/nabi.jpg",
        mp3: "spotify/musics/nabi.mp3",
        spotify: "https://open.spotify.com/playlist/6SdkbxaAZi2bTMSPbjIgnt"
    }
},{
    dataid: '2',
    image: 'profiles/dongdong1.png',
    name: '𝒊𝒌𝒈_𝒌𝒆𝒊𝒈𝒏',
    dataname: 'ikg_keign',
    role: '𝒇𝒐𝒖𝒏𝒅𝒆𝒓',
    birthday: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟖',
    databirthday: '2022-09-08',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_keign" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@keignnx" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@ingrid_ikigai" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@keignnx" },
        {name: "𝒄𝒐𝒅𝒎", icon: "apps/codm.webp", id: "@keignxha" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@keign." },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@keign_ikigai" }
    ],
    music: {
        title: "˚ ༘ ೀ⋆｡˚",
        artist: "@𝒌𝒆𝒊𝒈𝒏",
        cover: "spotify/icons/keign.jpg",
        mp3: "spotify/musics/keign.mp3",
        spotify: "https://open.spotify.com/playlist/6SdkbxaAZi2bTMSPbjIgnt"
    }
},{
    dataid: '3',
    image: 'profiles/dongdong2.png',
    name: '𝒊𝒌𝒈_𝒓𝒆𝒂𝒉𝒏',
    dataname: 'ikg_reahn',
    role: '𝒂𝒅𝒎𝒊𝒏',   
    birthday: '𝒋𝒖𝒏𝒆 𝟐𝟔',
    databirthday: '2022-06-26',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟒',
    datajoined: '2024-08-12',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_形無" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@Reahn_26" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@Klyaaaaa2874" },
        {name: "𝒎𝒍𝒃𝒃", icon: "apps/MLBB.jpg", id: "@Reahn" },
        {name: "𝒄𝒐𝒅𝒎", icon: "apps/codm.webp", id: "@Reahn" },
        {name: "𝒗𝒂𝒍𝒐𝒓𝒂𝒏𝒕", icon: "apps/valo.jpg", id: "@Reahn" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@msg_edb" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@ikigai_reahn" }
    ],
    music: {
        title: "𝑵𝒐𝒃𝒐𝒅𝒚 𝒈𝒆𝒕𝒔 𝒎𝒆",
        artist: "@𝒔𝒛𝒂",
        cover: "spotify/icons/reahn.jpg",
        mp3: "spotify/musics/reahn.mp3",
        spotify: "https://open.spotify.com/track/5Y35SjAfXjjG0sFQ3KOxmm?si=7lkRvvQmTaueb37E44cCzg&utm_source=copy-link"
    }
},{
    dataid: '4',
    image: 'profiles/speedy.png',
    name: '𝒊𝒌𝒈_𝒔𝒑𝒆𝒆𝒅𝒚',
    dataname: 'ikg_speedy',
    role: '𝒂𝒅𝒎𝒊𝒏',   
    birthday: '𝒇𝒆𝒃𝒓𝒖𝒂𝒓𝒚 𝟑',
    databirthday: '2022-02-03',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟒',
    datajoined: '2024-08-08',  
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Speedy" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@SpeedyTimeGaming" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:36315q9p" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@speedytime24" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@speedytimeofficial" }
    ]
},{
    dataid: '5',
    image: 'profiles/beau.png',
    name: '𝒊𝒌𝒈_𝒃𝒆𝒂𝒖',
    dataname: 'ikg_beau',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓', 
    birthday: '𝒏𝒐𝒗𝒆𝒎𝒃𝒆𝒓 𝟏𝟒',
    databirthday: '2022-11-14',
    joined: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-01-13',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_beau°" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@AstaYuno21" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:4pj4ftf" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@Meanne27" }
    ]
},{
    dataid: '6',
    image: 'profiles/lilith.png',
    name: '𝒊𝒌𝒈_𝒍𝒊𝒍𝒊𝒕𝒉',
    dataname: 'ikg_lilith',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',   
    birthday: '𝒏𝒐𝒗𝒆𝒎𝒃𝒆𝒓 𝟏𝟒',
    databirthday: '2022-11-14',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_lilith" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@xXLilithhXxx" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@lilith_ikigai" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1yq65q49" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@ikg_lilith" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@liliith_30" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@ikg_lilith" }
    ]
},{
    dataid: '7',
    image: 'profiles/dongdong1.png',
    name: '𝒊𝒌𝒈_𝒔𝒂𝒈𝒈𝒚',
    dataname: 'ikg_saggy',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',  
    databirthday: '2022-11-29',
    birthday: '𝒏𝒐𝒗𝒆𝒎𝒃𝒆𝒓 𝟐𝟗',
    joined: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-01-17',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_saggy" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@ishiiimerie" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@ishiiimerie" },
        {name: "𝒎𝒍𝒃𝒃", icon: "apps/MLBB.jpg", id: "@saggy" },
        {name: "𝒉𝒐𝒌", icon: "apps/hok.jpg", id: "@saggy" },
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@saggitariously" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@saggitariously_" }
    ],
    music: {
        title: "^^",
        artist: "@𝒊𝒔𝒉𝒊𝒊𝒎𝒆𝒓𝒊𝒆",
        cover: "spotify/icons/saggy.jpg",
        mp3: "spotify/musics/saggy.mp3",
        spotify: "https://open.spotify.com/playlist/570WVBLcoyfqSF2hj8huWS?si=NWUSZvweRseoiyTlazD_dA"
    }
},{
    dataid: '8',
    image: 'profiles/dongdong2.png',
    name: '𝒊𝒌𝒈_𝒍𝒖𝒄𝒊𝒐𝒕𝒂𝒏',
    dataname: 'ikg_luciotan',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒐𝒄𝒕𝒐𝒃𝒆𝒓 𝟏𝟏',
    databirthday: '2022-10-11',
    joined: '𝒂𝒑𝒓𝒊𝒍 𝟐𝟎𝟐𝟒',
    datajoined: '2024-04-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_luciotan" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:64ps3a1a" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@luciotan20th" }
    ]
},{
    dataid: '9',
    image: 'profiles/dongdong1.png',
    name: '𝒊𝒌𝒈_𝒋𝒊𝒏𝒕𝒂𝒓',
    dataname: 'ikg_jintar',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒚 𝟏𝟓',
    databirthday: '2022-05-15',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_jintar" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@jintaaar" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@jintar_ikigai" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1jjw3nfs" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@jintaaar" },
        {name: "𝒕𝒐𝒖𝒄𝒉", icon: "apps/touch.png", id: "@ikg_jintar" },
        {name: "𝒂𝒖𝒅𝒊𝒕𝒊𝒐𝒏", icon: "apps/audi.jpg", id: "@ikg_jintar" },
        {name: "𝒊𝒅𝒂𝒕𝒆", icon: "apps/idate.ico", id: "@ikg_jintar" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@jintaaar" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@ikg_jintar"}
    ],
    music: {
        title: ".",
        artist: "@𝒋𝒊𝒏𝒕𝒂𝒂𝒂𝒓",
        cover: "spotify/icons/jintar.jpg",
        mp3: "spotify/musics/jintar.mp3",
        spotify: "https://open.spotify.com/playlist/5ug8DfGCEVkwNhPINQ7hHz?si=iGlIC9NjTSeKisIp86WO3g"
    }
},{
    dataid: '10',
    image: 'profiles/ben.png',
    name: '𝒊𝒌𝒈_𝒃𝒆𝒏',
    dataname: 'ikg_ben',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒖𝒍𝒚 𝟐𝟕',
    databirthday: '2022-07-27',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-26',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_ben" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@henlo_im_ben" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@eggyben_" }
    ]
},{
    dataid: '11',
    image: 'profiles/anya.png',
    name: '𝒊𝒌𝒈_𝒂𝒏𝒚𝒂',
    dataname: 'ikg_anya',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒅𝒆𝒄𝒆𝒎𝒃𝒆𝒓 𝟐',
    databirthday: '2022-12-02',
    joined: '𝒎𝒂𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-05-29',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_あにゃ" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@sunzero04" },
    ],
    connections: [
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@anyadayo02" }
    ]
},{
    dataid: '12',
    image: 'profiles/nene.png',
    name: '𝒊𝒌𝒈_𝒏𝒆𝒏𝒆',
    dataname: 'ikg_nene',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟑𝟎',
    databirthday: '2022-08-30',
    joined: '𝒎𝒂𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-05-29',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_ねね" }
    ],
    connections: [
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@nenene_98" }
    ]
},{
    dataid: '13',
    image: 'profiles/slurry.png',
    name: '𝒊𝒌𝒈_𝒔𝒍𝒖𝒓𝒓𝒚',
    dataname: 'ikg_slurry',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒑𝒓𝒊𝒍 𝟏𝟎',
    databirthday: '2022-04-10',
    joined: '𝒋𝒖𝒏𝒆 𝟐𝟎𝟐𝟓',
    datajoined: '2025-06-21',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_명진" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:7n36211" }
    ]
},{
    dataid: '14',
    image: 'profiles/matrix.png',
    name: '𝒊𝒌𝒈_𝒎𝒂𝒕𝒓𝒊𝒙',
    dataname: 'ikg_matrix',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒚 𝟏𝟗',
    databirthday: '2022-05-19',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_matrix" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@Cmatrix_01" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@Cmatrix101" }

    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@cmatrix10" }
    ]
},{
    dataid: '15',
    image: 'profiles/neid.png',
    name: '𝒊𝒌𝒈_𝒏𝒆𝒊𝒅',
    dataname: 'ikg_neid',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒖𝒍𝒚 𝟏𝟓',
    databirthday: '2022-07-15',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-22',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Neid" },

    ]
},{
    dataid: '16',
    image: 'profiles/kazumi.png',
    name: '𝒊𝒌𝒈_𝒌𝒂𝒛𝒖𝒎𝒊',
    dataname: 'ikg_kazumi',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟏𝟔',
    databirthday: '2022-01-16',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-22',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_kazumi^" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:24tr4y2p" }

    ]
},{
    dataid: '17',
    image: 'profiles/jazz.png',
    name: '𝒊𝒌𝒈_𝒋𝒂𝒛𝒛',
    dataname: 'ikg_jazz',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',   
    birthday: '𝒐𝒄𝒕𝒐𝒃𝒆𝒓 𝟏𝟎',
    databirthday: '2022-10-10',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_ Latte." },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@Bxbyy_99" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1kz762jx" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@_notjazzz" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@_notlatte" }
    ]
},{
    dataid: '18',
    image: 'profiles/run.png',
    name: '𝒊𝒌𝒈_𝒓𝒖𝒏',
    dataname: 'ikg_run',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒚 𝟎𝟔',
    databirthday: '2022-05-06',
    joined: '𝒇𝒆𝒃𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-02-07',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_鲁恩" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@Ducky0250" },
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@runn_." },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@rundrunner" }
    ]
},{
    dataid: '19',
    image: 'profiles/shims.png',
    name: '𝒊𝒌𝒈_𝒔𝒉𝒊𝒎𝒔',
    dataname: 'ikg_shims',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒏𝒐𝒗𝒆𝒎𝒃𝒆𝒓 𝟏𝟖',
    databirthday: '2022-11-18',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_shims" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@hookie721" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1ydz5qf7" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@shims0825" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@ikg_shims" }
    ]
},{
    dataid: '20',
    image: 'profiles/ivy.png',
    name: '𝒊𝒌𝒈_𝒊𝒗𝒚',
    dataname: 'ikg_ivy',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒚 𝟏𝟖',
    databirthday: '2022-05-18',
    joined: '𝒇𝒆𝒃𝒓𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-02-17',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_ivy" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1vcm3gtb" }
    ],
    connections: [
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@ivyjvaski0" }
    ]
},{
    dataid: '21',
    image: 'profiles/light.png',
    name: '𝒊𝒌𝒈_𝒍𝒊𝒈𝒉𝒕',
    dataname: 'ikg_light',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟏𝟖',
    databirthday: '2022-08-18',
    joined: '𝒇𝒆𝒃𝒓𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-02-18',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_light" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:17cb3awn" }
    ]
},{
    dataid: '22',
    image: 'profiles/niki.png',
    name: '𝒊𝒌𝒈_𝒏𝒊𝒌𝒊',
    dataname: 'ikg_niki',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒐𝒄𝒕𝒐𝒃𝒆𝒓 𝟒',
    databirthday: '2022-10-04',
    joined: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-01-03',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_niki" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@uzamichan0" }
    ],
},{
    dataid: '23',
    image: 'profiles/hajime.png',
    name: '𝒊𝒌𝒈_𝒉𝒂𝒋𝒊𝒎𝒆',
    dataname: 'ikg_hajime',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒓𝒄𝒉 𝟏𝟗',
    databirthday: '2022-03-19',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_ney." },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@ikg_emijah" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1kej6201" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@cy_hajime_13672" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@ikg_ney" }
    ]
},{
    dataid: '24',
    image: 'profiles/scara.png',
    name: '𝒊𝒌𝒈_𝒔𝒄𝒂𝒓𝒂',
    dataname: 'ikg_scara',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒅𝒆𝒄𝒆𝒎𝒃𝒆𝒓 𝟏𝟕',
    databirthday: '2022-12-17',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟒',
    datajoined: '2024-08-12',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Scara" }
    ]
},{
    dataid: '25',
    image: 'profiles/akira.png',
    name: '𝒊𝒌𝒈_𝑨𝒌𝒊 𝑹𝒂',
    dataname: 'ikg_akira',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟏𝟒',
    databirthday: '2022-08-14',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟒',
    datajoined: '2024-08-12',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Aki Ra" }
    ]
},{
    dataid: '26',
    image: 'profiles/juswa.png',
    name: '𝒊𝒌𝒈_𝒋𝒖𝒔𝒘𝒂',
    dataname: 'ikg_juswa',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟖',
    databirthday: '2022-08-28',
    joined: '𝒇𝒆𝒃𝒓𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-02-07',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Juswaaa" }
    ]
},{
    dataid: '27',
    image: 'profiles/michie.png',
    name: '𝒊𝒌𝒈_𝒎𝒊𝒄𝒉𝒊𝒆',
    dataname: 'ikg_michie',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒖𝒏𝒆 𝟏𝟔',
    databirthday: '2022-06-16',
    joined: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-01-11',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_michiee" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1bms5qbw" }
    ]
},{
    dataid: '28',
    image: 'profiles/jewel.png',
    name: '𝒊𝒌𝒈_𝒋𝒆𝒘𝒆𝒍',
    dataname: 'ikg_jewel',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒓𝒄𝒉 𝟐𝟖',
    databirthday: '2022-03-28',
    joined: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-01-12',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_jewel" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1yqf62tf" }
    ]
},{
    dataid: '29',
    image: 'profiles/meowbun.png',
    name: '𝒊𝒌𝒈_𝒎𝒆𝒐𝒘𝒃𝒖𝒏',
    dataname: 'ikg_meowbun',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟏𝟎',
    databirthday: '2022-09-10',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_meowbun" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:47af4fz6" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@meowbunly" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@meowesomeee" }
    ]
},{
    dataid: '30',
    image: 'profiles/sze.png',
    name: '𝒊𝒌𝒈_𝑺𝒛𝒆',
    dataname: 'ikg_sze',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐',
    databirthday: '2022-08-02',
    joined: '𝒎𝒂𝒚 𝟐𝟎𝟐𝟓',
    datajoined: '2025-05-08',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Sze" }
    ]
},{
    dataid: '31',
    image: 'profiles/queen.png',
    name: '𝒊𝒌𝒈_𝒒𝒖𝒆𝒆𝒏',
    dataname: 'ikg_queen',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐',
    databirthday: '2022-08-11',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Queen" }
    ]
},{
    dataid: '32',
    image: 'profiles/yeji.png',
    name: '𝒊𝒌𝒈_𝒚𝒆𝒋𝒊',
    dataname: 'ikg_yeji',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟔',
    databirthday: '2022-01-06',
    joined: '𝒂𝒑𝒓𝒊𝒍 𝟐𝟎𝟐𝟓',
    datajoined: '2025-04-11',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Yeji" }
    ]
},{
    dataid: '33',
    image: 'profiles/iris.png',
    name: '𝒊𝒌𝒈_𝒊𝒓𝒊𝒔',
    dataname: 'ikg_iris',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒑𝒓𝒊𝒍 𝟏𝟑',
    databirthday: '2022-04-13',
    joined: '𝒂𝒑𝒓𝒊𝒍 𝟐𝟎𝟐𝟓',
    datajoined: '2025-04-11',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_íris" }
    ]
},{
    dataid: '34',
    image: 'profiles/yza.png',
    name: '𝒊𝒌𝒈_𝒚𝒛𝒂',
    dataname: 'ikg_yza',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒅𝒆𝒄𝒆𝒎𝒃𝒆𝒓 𝟏𝟎',
    databirthday: '2022-12-10',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟒',
    datajoined: '2024-08-08',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_yza" }
    ]
},{
    dataid: '35',
    image: 'profiles/rae.png',
    name: '𝒊𝒌𝒈_𝒓𝒂𝒆',
    dataname: 'ikg_rae',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟏𝟔',
    databirthday: '2022-9-16',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟒',
    datajoined: '2024-08-08',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_rae" }
    ]
},{
    dataid: '36',
    image: 'profiles/sky.png',
    name: '𝒊𝒌𝒈_𝒔𝒌𝒚',
    dataname: 'ikg_sky',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟐𝟏',
    databirthday: '2022-01-21',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟒',
    datajoined: '2024-08-08',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_sky" }
    ]
},{
    dataid: '37',
    image: 'profiles/meatball.png',
    name: '𝒊𝒌𝒈_𝒎𝒆𝒂𝒕𝒃𝒂𝒍𝒍',
    dataname: 'ikg_meatball',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒐𝒄𝒕𝒐𝒃𝒆𝒓 𝟏𝟎',
    databirthday: '2022-10-10',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟒',
    datajoined: '2024-08-08',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_MeatBall" }
    ]
},{
    dataid: '38',
    image: 'profiles/m_trese.png',
    name: '𝒊𝒌𝒈_𝒎_𝒕𝒓𝒆𝒔𝒆',
    dataname: 'ikg_m_trese',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟒',
    datajoined: '2024-08-08',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@Ikg_m_trese" }
    ]
},{
    dataid: '39',
    image: 'profiles/kizaru.png',
    name: '𝒊𝒌𝒈_𝒌𝒊𝒛𝒂𝒓𝒖',
    dataname: 'ikg_kizaru',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒑𝒓𝒊𝒍 𝟏𝟕',
    databirthday: '2022-04-17',
    joined: '𝒎𝒂𝒓𝒄𝒉 𝟐𝟎𝟐𝟓',
    datajoined: '2025-03-16',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Kizaru" }
    ]
},{
    dataid: '40',
    image: 'profiles/kyo.png',
    name: '𝒊𝒌𝒈_𝒌𝒚𝒐',
    dataname: 'ikg_kyo',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_kyo" }
    ]
},{
    dataid: '41',
    image: 'profiles/yumi.png',
    name: '𝒊𝒌𝒈_𝒚𝒖𝒎𝒊',
    dataname: 'ikg_yumi',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒚 𝟗',
    databirthday: '2022-05-09',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Nutella" }
    ]
},{
    dataid: '42',
    image: 'profiles/ely.png',
    name: '𝒊𝒌𝒈_𝒆𝒍𝒚',
    dataname: 'ikg_ely',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟏𝟓',
    databirthday: '2022-08-15',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Ely." }
    ]
},{
    dataid: '43',
    image: 'profiles/jnice.png',
    name: '𝒊𝒌𝒈_𝒋𝒏𝒊𝒄𝒆',
    dataname: 'ikg_jnice',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟗',
    databirthday: '2022-08-29',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Jnicee" }
    ]
},{
    dataid: '44',
    image: 'profiles/dongdong1.png',
    name: '𝒊𝒌𝒈_𝒊𝒄𝒆',
    dataname: 'ikg_ice',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
},{
    dataid: '45',
    image: 'profiles/dongdong1.png',
    name: '𝒊𝒌𝒈_𝒂𝒗𝒆𝒍𝒍𝒊𝒏𝒙',
    dataname: 'ikg_avellinx',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_avellinx" }
    ]
},{
    dataid: '46',
    image: 'profiles/jejemon.png',
    name: '𝒊𝒌𝒈_𝒋𝒆𝒋𝒆𝒎𝒐𝒏',
    dataname: 'ikg_jejemon',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_jejemon" }
    ]
},{
    dataid: '47',
    image: 'profiles/julia.png',
    name: '𝒊𝒌𝒈_𝒋𝒖𝒍𝒊𝒂',
    dataname: 'ikg_julia',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Julia" }
    ]
},{
    dataid: '48',
    image: 'profiles/asten.png',
    name: '𝒊𝒌𝒈_𝒂𝒔𝒕𝒆𝒏',
    dataname: 'ikg_asten',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒇𝒆𝒃𝒓𝒖𝒂𝒓𝒚 𝟗',
    databirthday: '2022-02-09',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_asten" }
    ]
},{
    dataid: '49',
    image: 'profiles/beizu.png',
    name: '𝒊𝒌𝒈_𝒃𝒆𝒊𝒛𝒖',
    dataname: 'ikg_beizu',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟏𝟖',
    databirthday: '2022-01-18',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Beizu" }
    ]
},{
    dataid: '50',
    image: 'profiles/aia.png',
    name: '𝒊𝒌𝒈_𝒂𝒊𝒂',
    dataname: 'ikg_aia',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐',
    databirthday: '2022-09-02',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_aia" }
    ]
},{
    dataid: '51',
    image: 'profiles/celia.png',
    name: '𝒊𝒌𝒈_𝒄𝒆𝒍𝒊𝒂',
    dataname: 'ikg_celia',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒖𝒏𝒆 𝟏',
    databirthday: '2022-06-01',
    joined: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-01-13',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Celia" }
    ]
},{
    dataid: '51',
    image: 'profiles/babiijaa.png',
    name: '𝒊𝒌𝒈_𝒃𝒂𝒃𝒊𝒊𝒋𝒂𝒂',
    dataname: 'ikg_babiijaa',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒚 𝟐𝟔',
    databirthday: '2022-05-26',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_BabiiJaa" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@BabiiJaa26" }
    ]
},{
    dataid: '52',
    image: 'profiles/aisen.png',
    name: '𝒊𝒌𝒈_𝒂𝒊𝒔𝒆𝒏',
    dataname: 'ikg_aisen',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟏𝟏',
    databirthday: '2022-01-11',
    joined: '𝒎𝒂𝒓𝒄𝒉 𝟐𝟎𝟐𝟒',
    datajoined: '2024-03-10',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_死Shi" },
        {name: "𝒎𝒊𝒏𝒆𝒄𝒓𝒂𝒇𝒕", icon: "apps/minecraft.webp", id: "@aisenn3239" }
    ]
},{
    dataid: '53',
    image: 'profiles/nishi.png',
    name: '𝒊𝒌𝒈_𝒏𝒊𝒔𝒉𝒊',
    dataname: 'ikg_nishi',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒓𝒄𝒉 𝟔',
    databirthday: '2022-03-06',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Nishi" }
    ]
},{
    dataid: '54',
    image: 'profiles/miya.png',
    name: '𝒊𝒌𝒈_𝒎𝒊𝒚𝒂',
    dataname: 'ikg_miya',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟐𝟏',
    databirthday: '2022-01-21',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_miya" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@Yukari_Ackerman2" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@miya_ikigai" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1q804qqc" },
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@sereia_xly" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@shirorex_" }
    ]
},{
    dataid: '55',
    image: 'profiles/habie.png',
    name: '𝒊𝒌𝒈_𝒉𝒂𝒃𝒊𝒆',
    dataname: 'ikg_habie',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒑𝒓𝒊𝒍 𝟐𝟐',
    databirthday: '2022-04-22',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_habie" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@strawberrykn0ts" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@habie_ikigai" }
    ]
},{
    dataid: '56',
    image: 'profiles/dongdong1.png',
    name: '𝒊𝒌𝒈_𝒌𝒂𝒊𝒗𝒐𝒓',
    dataname: 'ikg_kaivor',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒐𝒄𝒕𝒐𝒃𝒆𝒓 𝟐𝟑',
    databirthday: '2022-10-23',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_kaivor" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@renzified_0" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@akashi_ikigai" }
    ],
    connections: [
        {name: "𝒅𝒊𝒔𝒄𝒐𝒓𝒅", icon: "apps/discord.jpg", id: "@rnzsmt_" },
        {name: "𝒕𝒊𝒌𝒕𝒐𝒌", icon: "apps/tiktok.avif", id: "@rnzsmt" }
    ],
    music: {
        title: "𝑴𝒆𝒓𝒓𝒚 𝑪𝒉𝒓𝒊𝒔𝒕𝒎𝒂𝒔, 𝒊 𝒎𝒊𝒔𝒔 𝒚𝒐𝒖",
        artist: "@𝒂𝒍𝒆𝒙 𝒄𝒉𝒓𝒊𝒄𝒉𝒕𝒐𝒏",
        cover: "spotify/icons/kaivor.jpg",
        mp3: "spotify/musics/kaivor.mp3",
        spotify: "https://open.spotify.com/track/0XYALktGbnHJ1TdW0Oz5KN?si=c015a3ede4bf41bb"
    }
},{
    dataid: '57',
    image: 'profiles/dongdong1.png',
    name: '𝒊𝒌𝒈_𝒘𝒂𝒃𝒂𝒎',
    dataname: 'ikg_wabam',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_wabam" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@Asterion010" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@wabam_ikigai" }
    ]
},{
    dataid: '58',
    image: 'profiles/paw.png',
    name: '𝒊𝒌𝒈_𝒑𝒂𝒘',
    dataname: 'ikg_paw',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒖𝒍𝒚 𝟕',
    databirthday: '2022-07-07',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_rimuu" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@paw_ikigai" }
    ]
},{
    dataid: '59',
    image: 'profiles/pier.png',
    name: '𝒊𝒌𝒈_𝒑𝒊𝒆𝒓',
    dataname: 'ikg_pier',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒎𝒂𝒚 𝟏𝟏',
    databirthday: '2022-05-11',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_cyrie" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@pier_ikigai" }
    ]
},{
    dataid: '60',
    image: 'profiles/yhumx.png',
    name: '𝒊𝒌𝒈_𝒚𝒉𝒖𝒎𝒙',
    dataname: 'ikg_yhumx',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_yhumx" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@aimi_ikigai" }
    ]
},{
    dataid: '61',
    image: 'profiles/xeno.png',
    name: '𝒊𝒌𝒈_𝒙𝒆𝒏𝒐',
    dataname: 'ikg_xeno',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_xino" },
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@OverDr1nk_pH" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@xeno_ikigai" }
    ]
},{
    dataid: '62',
    image: 'profiles/lina.png',
    name: '𝒊𝒌𝒈_𝒍𝒊𝒏𝒂',
    dataname: 'ikg_lina',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟏𝟔',
    databirthday: '2022-09-16',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',
    
    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_lina" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@lian_ikigai" }
    ]
},{
    dataid: '63',
    image: 'profiles/koa.png',
    name: '𝒊𝒌𝒈_𝒌𝒐𝒂',
    dataname: 'ikg_koa',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',

    games: [
        {name: "𝒓𝒐𝒃𝒍𝒐𝒙", icon: "apps/roblox.webp", id: "@koa_ikigai" },
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@koa_ikigai" }
    ]
},{
    dataid: '64',
    image: 'profiles/khayyi.png',
    name: '𝒊𝒌𝒈_𝒌𝒉𝒂𝒚𝒚𝒊',
    dataname: 'ikg_khayyi',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',

    games: [
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@khayyi_ikigai" }
    ]
},{
    dataid: '65',
    image: 'profiles/klerh.png',
    name: '𝒊𝒌𝒈_𝒌𝒍𝒆𝒓𝒉',
    dataname: 'ikg_klerh',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟐',
    datajoined: '2022-07-01',

    games: [
        {name: "𝒃𝒖𝒅", icon: "apps/bud.jpg", id: "@klerh_ikigai" },
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:5s774y9j" }
    ]
},{
    dataid: '66',
    image: 'profiles/vera.png',
    name: '𝒊𝒌𝒈_𝒗𝒆𝒓𝒂',
    dataname: 'ikg_vera',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_アイカ" }
    ]
},{
    dataid: '67',
    image: 'profiles/shai.png',
    name: '𝒊𝒌𝒈_𝒔𝒉𝒂𝒊',
    dataname: 'ikg_shai',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒂𝒑𝒓𝒊𝒍 𝟐𝟎𝟐𝟒',
    datajoined: '2024-04-28',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_shai" }
    ]
},{
    dataid: '68',
    image: 'profiles/shaii.png',
    name: '𝒊𝒌𝒈_𝒔𝒉𝒂𝒊𝒊.',
    dataname: 'ikg_shaii',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟏𝟎',
    databirthday: '2022-08-10',
    joined: '𝒋𝒖𝒍𝒚 𝟐𝟎𝟐𝟒',
    datajoined: '2024-07-12',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_shàii." }
    ]
},{
    dataid: '69',
    image: 'profiles/cyril.png',
    name: '𝒊𝒌𝒈_𝒄𝒚𝒓𝒊𝒍',
    dataname: 'ikg_cyril',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '𝒋𝒖𝒏𝒆 𝟓',
    databirthday: '2022-06-05',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@#Dikg_Cyril" }
    ]
},{
    dataid: '70',
    image: 'profiles/keirah.png',
    name: '𝒊𝒌𝒈_𝒌𝒆𝒊𝒓𝒂𝒉',
    dataname: 'ikg_keirah',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_Keirah" }
    ]
},{
    dataid: '71',
    image: 'profiles/zsasha.png',
    name: '𝒊𝒌𝒈_𝒛𝒔𝒂𝒔𝒉𝒂',
    dataname: 'ikg_zsasha',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_zsasha" }
    ]
},{
    dataid: '72',
    image: 'profiles/cherry.png',
    name: '𝒊𝒌𝒈_𝒄𝒉𝒆𝒓𝒓𝒚',
    dataname: 'ikg_cherry',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_cherry" }
    ]
},{
    dataid: '73',
    image: 'profiles/aria.png',
    name: '𝒊𝒌𝒈_𝒂𝒓𝒊𝒂',
    dataname: 'ikg_aria',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_aria." }
    ]
},{
    dataid: '74',
    image: 'profiles/vivy.png',
    name: '𝒊𝒌𝒈_𝒗𝒊𝒗𝒚',
    dataname: 'ikg_vivy',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_vivy" }
    ]
},{
    dataid: '75',
    image: 'profiles/lele.png',
    name: '𝒊𝒌𝒈_𝒍𝒆𝒍𝒆',
    dataname: 'ikg_lele',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_lele" }
    ]
},{
    dataid: '76',
    image: 'profiles/dongdong1.png',
    name: '𝒊𝒌𝒈_𝟏𝟔𝟏',
    dataname: 'ikg_161',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_161" }
    ]
},{
    dataid: '77',
    image: 'profiles/wwl.png',
    name: '𝒊𝒌𝒈_𝒘𝒘𝒍',
    dataname: 'ikg_wwl',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_wwl" }
    ]
},{
    dataid: '78',
    image: 'profiles/hami.png',
    name: '𝒊𝒌𝒈_𝒉𝒂𝒎𝒊',
    dataname: 'ikg_hami',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_hami" }
    ]
},{
    dataid: '79',
    image: 'profiles/nakyum.png',
    name: '𝒊𝒌𝒈_𝒏𝒂𝒌𝒚𝒖𝒎',
    dataname: 'ikg_nakyum',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_nakyum" }
    ]
},{
    dataid: '80',
    image: 'profiles/jiroshi.png',
    name: '𝒊𝒌𝒈_𝒋𝒊𝒓𝒐𝒔𝒉𝒊',
    dataname: 'ikg_jiroshi',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒐𝒄𝒕𝒐𝒃𝒆𝒓 𝟐𝟎𝟐𝟑',
    datajoined: '2023-10-16',
},{
    dataid: '81',
    image: 'profiles/shopao.png',
    name: '𝒊𝒌𝒈_𝒔𝒉𝒐𝒑𝒂𝒐',
    dataname: 'ikg_shopao',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒂𝒖𝒈𝒖𝒔𝒕 𝟐𝟎𝟐𝟓',
    datajoined: '2025-08-02',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_shopao" }
    ]
},{
    dataid: '82',
    image: 'profiles/heaven.png',
    name: '𝒊𝒌𝒈_𝒉𝒆𝒂𝒗𝒆𝒏',
    dataname: 'ikg_heaven',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_heaven" }
    ]
},{
    dataid: '83',
    image: 'profiles/gean.png',
    name: '𝒊𝒌𝒈_𝒈𝒆𝒂𝒏',
    dataname: 'ikg_gean',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_geanx" }
    ]
},{
    dataid: '84',
    image: 'profiles/kael.png',
    name: '𝒊𝒌𝒈_𝒌𝒂𝒆𝒍',
    dataname: 'ikg_kael',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒋𝒂𝒏𝒖𝒂𝒓𝒚 𝟐𝟎𝟐𝟔',
    datajoined: '2026-01-07',

    games: [
        {name: "𝒉𝒆𝒂𝒓𝒕𝒐𝒑𝒊𝒂", icon: "apps/Heartopia.webp", id: "UID:1z6a3shr" }
    ]
},{
    dataid: '85',
    image: 'profiles/moon.png',
    name: '𝒊𝒌𝒈_𝒎𝒐𝒐𝒏',
    dataname: 'ikg_moon',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_moon" },
    ]
},{
    dataid: '86',
    image: 'profiles/litberar.png',
    name: '𝒊𝒌𝒈_𝒍𝒊𝒕𝒃𝒆𝒓𝒂𝒓',
    dataname: 'ikg_litberar',
    role: '𝒎𝒆𝒎𝒃𝒆𝒓',
    birthday: '*',
    databirthday: '2022-12-31',
    joined: '𝒔𝒆𝒑𝒕𝒆𝒎𝒃𝒆𝒓 𝟐𝟎𝟐𝟒',
    datajoined: '2024-09-01',

    games: [
        {name: "𝒆𝒈𝒈𝒚 𝒑𝒂𝒓𝒕𝒚", icon: "apps/eggyparty.png", id: "@ikg_LiTbErAr" },
    ]
}];

